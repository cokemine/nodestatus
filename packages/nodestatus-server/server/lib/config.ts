...
const program = new Command()
  ...
  .addOption(createOption('-pi, --ping-interval <pingInterval>', 'Ping interval').env('PING_INTERVAL').default(30).argParser(parseInt))
  ...
  .addOption(createOption('-pt, --push-timeout <pushTimeout>', 'Push timeout').env('PUSH_TIMEOUT').default(120).argParser(parseInt))
  .addOption(createOption('-pd, --push-delay <pushDelay>', 'Push delay').env('PUSH_DELAY').default(15).argParser(parseInt))

export function parseLoad(num: number): number {
  const load = Number(num.toFixed(2));
  return load % 1 ? load : Math.round(load);
}

export function parseUpdateTime(updated: number | undefined): string {
  if (!updated)
    return '从未';
...
export function parseUptime(uptime: number): string {
  if (uptime >= 86400)
